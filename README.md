...
```console
$ reshowcase start --entry=path/to/Demo.js --styles=path/to/styles.css
```

### To build bundle:

```console
$ reshowcase build --entry=path/to/Demo.js --styles=path/to/styles.css --output=path/to/bundle
```

The optional `--styles` file can contain CSS generated by `styled-ppx.generate`.
Reshowcase bundles it with the demo and adds it to both generated HTML pages.
